...
// 1 minute = 1000 pseudo-token convention. It reports false when the duration
// cannot be billed on at all — negative, non-finite, or implausibly large — so
// the caller falls back to the duration this gateway measured from the uploaded
// file itself, which is the one input it can actually vouch for.
func asrPromptTokens(duration float64) (int, bool) {
	if math.IsNaN(duration) || math.IsInf(duration, 0) {
		return 0, false
	}
	if duration <= 0 || duration > maxBillableASRSeconds {
		return 0, false
	}
